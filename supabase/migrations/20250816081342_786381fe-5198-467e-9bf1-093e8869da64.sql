-- Allow matched users to see each other's basic profile info (name, bio)
-- Update existing policy to be more explicit about what data is accessible
DROP POLICY IF EXISTS "Realtors can view renter preferences through matches" ON public.profiles;

-- Create policy that allows matched users to see each other's names and bios
CREATE POLICY "Matched users can view each other's basic profiles" 
ON public.profiles 
FOR SELECT 
USING (
...
-- Create function to get realtor contact info for renters (full access)
...
-- Create function to get renter basic info for realtors (no phone/email)
